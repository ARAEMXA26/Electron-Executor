...
async function executeLua(scriptContent, scriptName, gameInfo, logCallback) {
  ...
  // Run the Lua evaluation asynchronously using Promise to let the Electron main thread process events
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        await lua.doString(`
          ...
          -- Fix: create gethui mock to prevent scripts from erroring or yielding indefinitely
          function gethui()
          ...
          -- Fix: mock hookmetamethod to allow scripts targeting game's index metamethods
