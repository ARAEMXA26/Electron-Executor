...
npx electron-packager . "Electron Executor" --platform=darwin --overwrite --out=dist --icon=public/logo.icns --app-bundle-id=com.araemxa.electron-executor --ignore='^/dist($|/)|^/\.git($|/)|^/\.next($|/)|^/app($|/)|^/components($|/)|^/src($|/)|^/postcss\.config\.js$|^/next\.config\.js$|^/jsconfig\.json$|^/install\.sh$|^/setup-roblox\.sh$|^/package-lock\.json$|^/README\.md$|^/\.gitignore$|^/\.DS_Store$' 2>&1 | tail -3 || fail "Failed to package Electron application."
